...
const STEPS: Step[] = [
  ...
  {
    id: "requisites",
    title: "Реквизиты клиента и DaData",
    icon: Building2,
    summary: "Паспорт для физлиц, ИНН/КПП/ОГРН для юрлиц — попадают в договор автоматически.",
    details: (
      <>
        <p>
          В карточке клиента переключатель <b>Физлицо / Юрлицо</b> определяет блок реквизитов:
        </p>
        <ul className="list-disc pl-5 mt-2 space-y-1">
          <li><b>Физлицо</b> — ФИО, дата рождения, серия/номер паспорта, кем и когда выдан, адрес регистрации.</li>
          <li><b>Юрлицо</b> — название, ИНН, КПП, ОГРН, юр. адрес, ФИО и должность руководителя.</li>
        </ul>
        <p className="mt-2">
          Для юрлица нажмите <b>«Синхронизировать»</b> рядом с полем ИНН — данные подтянутся из <b>DaData</b> (ЕГРЮЛ/ЕГРИП) автоматически: название, КПП, ОГРН, адрес и руководитель.
        </p>
        <p className="mt-2">
          Эти поля затем подставляются в шапку и в раздел «Принципал» договора Алиста — без ручного копирования.
        </p>
      </>
    ),
    tasks: [
      { id: "open_clients_req", label: "Открыть карточку клиента", href: "/admin/clients" },
      { id: "sync_inn", label: "Синхронизировал юрлицо по ИНН" },
    ],
  },
  {
    id: "pipeline",
  ...
  },
  {
    ...
    title: "Документы",
    icon: FileText,
    summary: "Фирменный договор Алиста (.docx) — генерится в один клик с предпросмотром.",
    details: (
      <>
        <p>
          В разделе <b>«Документы → Шаблоны»</b> всегда есть системный <b>«Агентский договор Алиста (.docx)»</b> — иконка ★. Это полноценный 21-страничный договор с логотипом, шапкой, таблицами и реквизитами Алисты.
        </p>
        <p className="mt-2">
          Открыть его можно тремя способами:
        </p>
        <ul className="list-disc pl-5 mt-1 space-y-1">
          <li><b>«Документы → Файлы»</b> → кнопка <b>«Договор Алиста (.docx)»</b>.</li>
          <li><b>«Документы → Шаблоны»</b> → иконка глаза/документа у системной строки.</li>
          <li><b>Карточка сделки → «Документы» → «Договор Алиста»</b> — сделка уже подставлена.</li>
        </ul>
        <p className="mt-2">
          В диалоге: выберите сделку, тип принципала (физ/юрлицо подтянется из карточки клиента), № договора (генерится автоматически в формате <code>АЛ-YYYYMMDD-NNN</code>) и дату. Кнопка <b>«Предпросмотр»</b> сразу показывает готовый договор внутри окна — можно проверить шапку, блок Принципала, реквизиты на стр. 15. Кнопка <b>«Скачать .docx»</b> отдаёт файл с подставленными данными клиента.
        </p>
      </>
    ),
    tasks: [
      { id: "open_docs", label: "Открыть «Документы»", href: "/admin/documents" },
      { id: "preview_alista", label: "Открыл предпросмотр договора Алиста" },
      { id: "downloaded_alista", label: "Скачал .docx и проверил реквизиты" },
    ],
  },
  {
    id: "catalog",
